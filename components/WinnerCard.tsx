...
export function WinnerCard({ winner }: { winner: WinnerRecord }) {
  return (
    <Link href={`/raffle/${winner.raffleId}`}>
      <Card className="glass-card cursor-pointer transition hover:border-primary/50 hover:shadow-lg">
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-text font-mono">{formatAddress(winner.winner)}</p>
          </div>
          <div className="flex items-center justify-between text-sm text-muted">
            <span>Prize</span>
            <span className="text-text font-semibold">{winner.prize} ETH</span>
          </div>
